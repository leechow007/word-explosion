...
把清洗好的词表放到 **`词爆.app` 同级目录**，命名为 `wordpop-import.txt`（或 `.csv`），
下次启动 App 会自动完成导入：

1. 解析 → 去重 → 合并进「我的词本」→ 自动切换词源
2. 导入结果写入同目录 `wordpop-import.log`
3. 导入完成后自动删除该文件（一次性，避免重复导入）

也可以用环境变量指定任意路径：

```bash
WORDPOP_IMPORT="/path/to/words.txt" open "dist/词爆.app"
```

> 第三方词库（如墨墨导出的雅思词汇）版权归原出处，请勿提交到公开仓库；
> `imports/` 与 `dist/` 已在 `.gitignore` 中忽略。
